...
describe("Parse 64-bit signed integer", () => {
    ...
    describe("big-endian", () => {
        ...
        it("should decode", () => {
            ...
            assert.equal(Token.INT64_BE.get(Buffer.from('\x00\x00\x00\x00\x00\x00\x00\x00', 'binary'), 0), BigInt(0));
            assert.equal(Token.INT64_BE.get(Buffer.from('\xff\xff\xff\xff\xff\xff\xff\xff', 'binary'), 0), BigInt(-1));
    ...
    describe("little-endian", () => {
        ...
        it("should decode", () => {
            ...
            assert.equal(Token.INT64_LE.get(buf, 0), BigInt(0));
            assert.equal(Token.INT64_LE.get(buf, 8), BigInt(-1));

            buf = Buffer.from('\xaa\xcc\xdd\xee\xbb\xff\x00\x00\xbb\xcc\xdd\xee\xbb\xff\x00\x00', 'binary');
            assert.equal(Token.INT64_LE.get(buf, 0), BigInt(0x0000ffbbeeddccaa));
            assert.equal(Token.INT64_LE.get(buf, 8), BigInt(0x0000ffbbeeddccbb));
